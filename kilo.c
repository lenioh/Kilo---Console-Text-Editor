...
int editor_syntax_to_color(int hl) {
    switch (hl) {
        ...
        case HL_MATCH:
            return 34;                  // blue
        default:
            return 37;                  // default greyish color
    }
}
...
void editor_insert_newline() {
    ...
    if (E.cx == 0) {
        ...
        // Split the current row into two rows
        editor_insert_row(E.cy + 1, &row->chars[E.cx], row->size - E.cx);
        row = &E.row[E.cy];  // Row might be reallocated, so update the pointer
        row->size = E.cx;
        row->chars[row->size] = '\0';
        editor_update_row(row);
    } else {
...
void editor_save() {
    if (E.filename == NULL) {
        ...
        if (E.filename == NULL) {
            editor_set_status_message("Saving aborted");
            return;
        }
        editor_select_syntax_highlight();
    }
...
void editor_find() {
    ...
    char *query = editor_prompt("Search: %s (Enter/ESC to cancel, Arrows to move)", editor_find_callback);
...
void editor_draw_statusbar(struct abuf *ab) {
    ...
    int len = snprintf(status, sizeof(status),
                       "%.20s - %d lines %s", E.filename ? E.filename : "[No Name]", E.numrows, 
                       E.dirty ? "(modified)" : "");
    int rlen = snprintf(rstatus, sizeof(rstatus),
                        "%s | %d/%d", E.syntax ? E.syntax->filetype : "no filetype", E.cy + 1, E.numrows);
    if (len > E.screencols) {
        len = E.screencols;
    }
